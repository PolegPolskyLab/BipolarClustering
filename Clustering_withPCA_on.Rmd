...
df_sum_clust_hc_pc <- bp_Zsum_clean %>%
  select(AUC, MeanTransient, TS2, riseT, TShift, ID, isSAC) %>% #riseT caused the matrix to be singular
...
res.nb <- bp_Zsum_clean %>%
  select(AUC, MeanTransient, TS2, riseT, TShift, ID, isSAC) %>%
...
df_sum_clust_hc_pc %>%
  select(Info, Cluster) %>%
  unique() %>%
  write.csv("/Users/Sam/Desktop/BP_Clustering/Clustered_on_final.csv")
write.csv(res.nb$All.index, "/Users/Sam/Desktop/BP_Clustering/Clustered_on_cindex.csv")
